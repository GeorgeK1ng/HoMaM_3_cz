; Heroes 3 Complete - Instalátor češtiny
      ...
      if (FR.Attributes and FILE_ATTRIBUTE_DIRECTORY) <> 0 then begin
        ...
        if (BaseName <> '.') and (BaseName <> '..') and (Lowercase(BaseName) <> 'maps') then begin
          ...
          if FileExists(LodPath) then begin
            ...
            LogInfo(Format('  Importováno: %d souborů.', [Imported]));
          end else begin
...
begin
  ...
  if not DirExists(SrcMaps) then begin
    Log('složka maps v payloadu nenalezena – přeskočeno.'); exit;
  end;
...
begin
  ...
  if CurPageID = wpSelectDir then begin
    ...
    if not DirExists(dataDir) then begin
      if MsgBox('Ve zvolené složce chybí podsložka "data". Jsi si jistý, že je to kořen instalace hry?',
